  ...
  Object.keys(fullSchema).forEach((val) => {
    if (expectationValues[val] === undefined) {
      ...
      if (fullSchema[val].default !== undefined) {
        ...
        fullSchema[val].expects !== undefined &&
        fullSchema[val].expects.length
      ) {
      ...
      if (dependency.indexOf("=") !== -1) {
        ...
        if (expectationValues[key] === undefined) {
          throw new ExpectationTypeException(
            "Important dependency not met: " + dependency
          );

          // Our processed value is an array and it includes the value we're looking for! Winner!
        } else if (
          Array.isArray(expectationValues[key].processedValue) &&
          expectationValues[key].processedValue.includes(val)
        ) {
...
exports.processExpectation = (expects = {}, value) => {
  ...
  switch (expects.type) {
    ...
    case "list":
      ...
      if (expects.possible_values !== undefined) {
        const difference = items.filter(
          (x) => !expects.possible_values.includes(x)
        );
        if (difference.length > 0) {
          // Unexpected value encountered
          result.message =
            "Invalid value encountered. Expected one of: " +
            expects.possible_values.join(",");
          return result;
        }
      }
    ...
    case "hex_color":
      if (
        !value.match(
          /^(?:(?:[0-9a-fA-F]{4}){1,2})|(?:(?:[0-9a-fA-F]{3}){1,2})$/
        )
      ) {
